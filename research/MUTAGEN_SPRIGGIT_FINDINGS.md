...
- **Pen / greenhouse terminal + hand scanner (product scope for data tools):** In play, the **module terminal** lists planet-tied products after typical **full-scan** gating. The **hand scanner** on wild fauna also shows **drops** and **outpost production** hints — so eligibility is **not** “terminal-only”; a **native** scanner/workshop-shared path exists (undocumented in our Papyrus/VMAD notes). For **ESM-only / planner tooling**, treat **full scan** as **player-known**; join **`FaunaCreation`** / planet fauna / loot / flora graphs; **cross-check** against scanner UI text in-game.
