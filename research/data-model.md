...
| Hand scanner UI (wild fauna) | **Observation:** overlay shows **loot/resource**, **biome**, etc., and **explicitly** states that the resource can be **produced at an outpost** — same product knowledge surfaced **outside** the pen terminal | Display strings confirmed in **`Starfield - Localization.ba2`** → **`strings/starfield_en.strings`** (e.g. **`Outpost production allowed`**, **`Temperament: …`**, **`Heals rapidly`**); mapping those tokens to **ESM conditions** / **AV** / **keywords** is still **TBD** ([outpost-papyrus-notes.md](outpost-papyrus-notes.md)) |
